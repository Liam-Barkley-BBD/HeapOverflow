...
public class ReplyServices {
    public static JsonNode getReplies(int page, int size) throws Exception {
        return HttpUtils
                .asyncGet(EnvUtils.getStringEnvOrThrow(EnvConstants.SERVER_URI) + ApiEndpointsConstants.API_REPLIES
                        + "?page=" + page + "&size=" + size)
                .join();
    }
    ...
    public static JsonNode getReplyById(String id) throws Exception {
        return HttpUtils
                .asyncGet(EnvUtils.getStringEnvOrThrow(EnvConstants.SERVER_URI) + ApiEndpointsConstants.API_REPLIES_ID
                        + id)
                .join();
    }
